import React from 'react';
import './latest-articles.css';
const Articles = () =>{
    return (
        <>
        <div className='latest-head'>
            Latest Articles
            <br />
            <div className='latest-hr'></div>
        </div>
        <div className='articles'>
            <div className='art-grid'>
                <div className='one-grid'>
                    <div className='one-grid-img'>
                    </div>
                    <div className='one-grid-cont'>
                        <div>
                            Catch waves with an adventure guide
                        </div>
                        <p>
                        Gujarat is vastly underrated and it's
                        a mystery to us why the region isn't 
                        more well.
                        </p>
                        <span><b>Travel</b> / August 21 2017</span>
                    </div>
                </div>
                <div className='one-grid'>
                    <div className='one-grid-img'>
                    </div>
                    <div className='one-grid-cont'>
                        <div>
                            Catch waves with an adventure guide
                        </div>
                        <p>
                        Gujarat is vastly underrated and it's
                        a mystery to us why the region isn't 
                        more well.
                        </p>
                        <span><b>Travel</b> / August 21 2017</span>
                    </div>
                </div>
                <div className='one-grid'>
                    <div className='one-grid-img'>

                    </div>
                    <div className='one-grid-cont'>
                        <div>
                            Catch waves with an adventure guide
                        </div>
                        <p>
                        Gujarat is vastly underrated and it's
                        a mystery to us why the region isn't 
                        more well.
                        </p>
                        <span><b>Travel</b> / August 21 2017</span>
                    </div>
                </div>
                <div className='one-grid'>
                    <div className='one-grid-img'>

                    </div>
                    <div className='one-grid-cont'>
                        <div>
                            Catch waves with an adventure guide
                        </div>
                        <p>
                        Gujarat is vastly underrated and it's
                        a mystery to us why the region isn't 
                        more well.
                        </p>
                        <span><b>Travel</b> / August 21 2017</span>
                    </div>
                </div>
                <div className='one-grid-more'>
                <img src="https://img.icons8.com/ios-filled/50/null/long-arrow-down.png"
                width='20' height='20' alt=''/>
                    Load More
                </div>
                <div className='two-grid'>
                    dcvfcbfbf
                </div>
            </div>
            <div className='art-grid2'>
                <div className='three-grid'>
                    <div><b>Advertisement</b></div>
                </div>
                <div className='four-grid'>
                <div className='four-head'>
                    Top Posts
                    <br />
                    <div className='four-hr'></div>
                </div>
                    <div className='four-grid-one'>
                        <div className='four-one-img'>
                        </div>
                        <div className='four-one-grid'>
                            <div className='four-one-cont'>
                                <p>Catch waves with an adventure guide</p>
                                <span><b>Travel</b> / August 21 2017</span>
                            </div>
                            <div className='four-one-num'>
                                1
                            </div>
                        </div>
                    </div>
                    <div className='four-grid-two'>
                        <div className='four-two-img'>
                        </div>
                        <div className='four-two-cont'>
                            <p>Catch waves with an adventure guide</p>
                            <span><b>Travel</b> / August 21 2017</span>
                        </div>
                        <div className='four-num'>
                            2
                        </div>
                    </div>
                    <div className='four-grid-two'>
                        <div className='four-two-img'>
                        </div>
                        <div className='four-two-cont'>
                            <p>Catch waves with an adventure guide</p>
                            <span><b>Travel</b> / August 21 2017</span>
                        </div>
                        <div className='four-num'>
                            3
                        </div>
                    </div>
                    <div className='four-grid-two'>
                        <div className='four-two-img'>
                            
                        </div>
                        <div className='four-two-cont'>
                            <p>Catch waves with an adventure guide</p>
                            <span><b>Travel</b> / August 21 2017</span>
                        </div>
                        <div className='four-num'>
                            4
                        </div>
                    </div>
                </div>
            </div>
        </div>
        </>
    )
}
export default Articles;